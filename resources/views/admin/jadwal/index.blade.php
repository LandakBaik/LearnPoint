                            ...
                            <tr class="hover:bg-indigo-50/30 transition-colors">
                                ...
                                <td class="py-4 px-4 text-right pr-6 whitespace-nowrap">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        @if($k->jadwal_url)
                                            <form action="{{ route('admin.jadwal.destroy_kelas', $k->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus foto jadwal kelas {{ $k->nama_kelas }}?');" class="inline">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <td class="py-4 px-4 text-right pr-6 whitespace-nowrap">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        @if($g->guruMapels->first() && $g->jadwal_url)
                                            <form action="{{ route('admin.jadwal.destroy_guru', $g->guruMapels->first()->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus foto jadwal mengajar guru {{ $g->nama }}?');" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="p-1.5 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded-lg transition-colors" title="Hapus Foto Jadwal">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
