        ...
        <form action="{{ route('kelas.update', $kelas->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select id="tingkatan" name="tingkatan" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50/50 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 font-medium text-gray-800">
                    ...
                    <option value="10" {{ old('tingkatan', $kelas->tingkatan) == '10' ? 'selected' : '' }}>Tingkat 10 (Kelas X)</option>
                    <option value="11" {{ old('tingkatan', $kelas->tingkatan) == '11' ? 'selected' : '' }}>Tingkat 11 (Kelas XI)</option>
                    <option value="12" {{ old('tingkatan', $kelas->tingkatan) == '12' ? 'selected' : '' }}>Tingkat 12 (Kelas XII)</option>
                </select>
                @error('tingkatan')
                    <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                @enderror
